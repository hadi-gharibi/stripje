...
class TestCategoryEncodersIntegration:
    ...
    def test_high_cardinality_with_unknown_categories(self):
        ...
        test_data_unknown = {
            'high_card_cat': 'new_unseen_category',
            'num_feature': 0.5
        }
        ...
        try:
            ...
            assert fast_pred_unknown in [0, 1], (
    ...
    def test_multiple_predictions_consistency(self):
        ...
        print(f"Prediction match rate: {match_rate:.2%} ({matches}/{len(test_indices)})")
    ...
    def test_empty_and_null_handling(self):
        ...
        assert match_rate >= 0.60, (
            f"Match rate too low: {match_rate:.2%} ({matches}/5)"
        )
